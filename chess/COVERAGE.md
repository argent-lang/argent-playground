...
| --- | --- | --- | --- |
| Side to move authorization | bounded | `Mux` authenticates the current signer before routing | `muxed_chess_routes_all_move_families` |
...
| Draw by agreement | bounded | a move can set `termination_action = 1`; `Mux.terminate` accepts the offer with `termination_action = 4`; an ordinary reply rejects it | `ordinary_move_can_offer_draw_and_return_to_mux`, `pending_draw_offer_can_be_accepted_on_next_mux_turn`, `ordinary_reply_rejects_pending_draw_offer_and_clears_draw_state` |
| Surrender / resignation | bounded | `Mux.terminate` with `termination_action = 3` emits terminal mux state for the conceding side | `surrender_routes_back_to_mux_with_terminal_status`, `surrender_can_settle_without_manual_request` |
| Timeout / liveness | bounded | mux timeout is opponent-signed, worker timeout is permissionless | `mux_timeout_awards_win_to_the_waiting_opponent`, `knight_worker_timeout_rescues_invalid_committed_state` |
...
| Value settlement on win / draw | bounded | `Settle` pays the stake to the winner or splits a draw, then emits two spendable `Player` outputs | `terminal_mux_settles_white_win_back_into_players`, `terminal_mux_settles_black_win_back_into_players`, `terminal_mux_settles_draw_back_into_players` |
